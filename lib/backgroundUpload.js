...
import { getValidAccessToken, forceRefresh } from './tokenManager';
...
async function uploadPhotoToDrive(photo, auditId, driveResources = null) {
    ...
    // Wrap in internal retry for token refresh
    try {
        ...
        if (err.message && (err.message.includes('Token expired') || err.message.includes('invalid'))) {
            console.log('[DIRECT UPLOAD] Token expired during upload. Forcing refresh and retrying...');

            try {
                // Force refresh token
                const newAccessToken = await forceRefresh();
                console.log('[DIRECT UPLOAD] Token refreshed. Retrying upload...');

                // Retry upload with new token
                const retryResult = await uploadBlobToDrive(
                    blob,
                    photo.filename,
                    folderId,
                    newAccessToken
                ...
                console.log(`[DIRECT UPLOAD] Retry success! File ID: ${retryResult.fileId}`);
                ...
                console.error('[DIRECT UPLOAD] Retry failed:', retryErr.message);
                throw retryErr; // Throw original or new error to be handled by queue
            }
        }
    ...
    try {
        while (uploadQueue.length > 0) {
            ...
            try {
                ...
                // WRAPPED in safeguard timeout to ensure queue never gets stuck
                const uploadPromise = uploadPhotoToDrive(photo, auditId, driveResources);
                const timeoutPromise = new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Upload operation timed out (safeguard)')), 70000)
                );
                ...
                if (retryCount < MAX_RETRIES - 1) {
                    ...
                    // We handle the removal logic here specially
                    shouldRemove = false; // Don't remove in finally, we act now
                    ...
                    // Max retries reached - mark as failed and REMOVE/IGNORE
                    console.log(`[UPLOAD] Max retries reached. Permanently removing photo ${photo.id} to unblock queue.`);
                    ...
                    // Reject the promise - caller should handle or ignore
                    // Wrapping in try-catch to ensure reject doesn't crash the loop
                    try {
                        reject(err);
                    } catch (rejectErr) {
                        console.error('[UPLOAD] Error calling reject handler:', rejectErr);
                    }

                    // shouldRemove stays true, so it will be shifted in finally
                }
            } finally {
                // CRITICAL: Ensure item is removed if not explicitly kept (retried)
                if (shouldRemove) {
                    uploadQueue.shift();
                }
            }
        ...
        isUploading = false;
        console.log('[DEBUG] processQueue: Finished processing, isUploading=false');
    }
