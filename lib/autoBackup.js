 ...
 */
export function startAutoBackup(audit, getLatestData, onBackupComplete = null) {
    ...
    let currentDriveResources = { ...audit.driveResources };

    console.log(`[BACKUP] Starting auto-backup for audit ${auditId}`);

    // Helper to persist backupFileId to IndexedDB
    const persistBackupId = async (newId) => {
        if (newId && newId !== backupFileId) {
            backupFileId = newId;
            currentDriveResources = { ...currentDriveResources, backupFileId: newId };

            try {
                // Dynamic import to avoid circular dependency
                const { updateAudit } = await import('./db');
                await updateAudit(auditId, { driveResources: currentDriveResources });
                console.log(`[BACKUP] Persisted backupFileId: ${newId}`);

                // Notify caller if callback provided
                if (onBackupComplete) {
                    onBackupComplete({ backupFileId: newId, timestamp: lastBackupTime });
                }
            } catch (err) {
                console.error('[BACKUP] Failed to persist backupFileId:', err.message);
            }
        } else if (onBackupComplete && newId) {
            // Still notify even if ID didn't change (backup completed)
            onBackupComplete({ backupFileId: newId, timestamp: lastBackupTime });
        }
    };
    ...
    const initialTimer = setTimeout(async () => {
        ...
        if (newBackupId) {
            await persistBackupId(newBackupId);
        }
    }, 30 * 1000);
