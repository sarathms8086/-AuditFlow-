...
async function submitAudit(audit, photos) {
    ...
    const photoData = await Promise.all(
        photos
            // Filter out failed photos - we skip them to allow submission to proceed
            .filter(photo => {
                if (photo.status === 'failed') {
                    console.warn(`[SYNC] Skipping failed photo: ${photo.filename} (${photo.itemId})`);
                    skippedPhotos++; // Track skipped count
                    return false;
                }
                return true;
            })
    ...
    // Filter out null photos (failed uploads) before sending to API
    const validPhotos = photoData.filter(p => p !== null);
    const failedPhotoCount = photoData.length - validPhotos.length;

    if (failedPhotoCount > 0) {
        console.warn(`[SYNC] ${failedPhotoCount} photos could not be uploaded - proceeding with ${validPhotos.length} valid photos`);
    }
    ...
    const response = await fetch(`${API_URL}/api/audit/submit`, {
        ...
        body: JSON.stringify({
            auditData: {
                ...
                failedPhotoCount, // Include count of failed photos for reporting
            },
            checklist: checklistWithResponses,
            photos: validPhotos, // Send only valid (non-null) photos
