 ...
 * Target: ~3MB max base64 (leaves room for other request data)
 * 
 * @param {Blob} blob - Original image blob
 * @param {number} maxSizeBytes - Maximum size in bytes (default 3MB for base64)
 * @returns {Promise<string>} - Compressed base64 string
 */
export async function compressImage(blob, maxSizeBytes = 3 * 1024 * 1024) {
    console.log(`[COMPRESS] Starting compression. Original size: ${blob.size} bytes`);

    // If already small enough, just convert to base64
    if (blob.size <= maxSizeBytes) {
        console.log('[COMPRESS] Image small enough, no compression needed');
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = async () => {
            ...
            // Base64 is ~33% larger than binary, so account for that
            const targetBinarySize = maxSizeBytes * 0.75;
            const currentBinarySize = blob.size;
            const scaleFactor = Math.sqrt(targetBinarySize / currentBinarySize);
            ...
            // Also cap at reasonable max dimensions
            const MAX_DIMENSION = 2000;
            if (width > MAX_DIMENSION || height > MAX_DIMENSION) {
                const dimScale = MAX_DIMENSION / Math.max(width, height);
                width = Math.round(width * dimScale);
                height = Math.round(height * dimScale);
            }

            console.log(`[COMPRESS] Resizing from ${img.width}x${img.height} to ${width}x${height}`);
            ...
            for (const quality of qualities) {
                ...
                console.log(`[COMPRESS] Quality ${quality}: ${Math.round(sizeBytes / 1024)}KB`);

                if (sizeBytes <= maxSizeBytes) {
                    console.log(`[COMPRESS] Success! Final size: ${Math.round(sizeBytes / 1024)}KB`);
                    resolve(base64);
                    return;
                }
            }

            // If still too large, use lowest quality result
            const finalDataUrl = canvas.toDataURL('image/jpeg', 0.3);
            const finalBase64 = finalDataUrl.split(',')[1];
            console.log(`[COMPRESS] Using minimum quality. Final size: ${Math.round(finalBase64.length / 1024)}KB`);
            resolve(finalBase64);
        };
...
/**
 * Cleanup photo blobs after audit is successfully synced
 * This is called from sync.js after a successful audit submission
 * Blobs are deleted but metadata is kept for reference
 */
export async function cleanupSyncedAuditPhotos(auditId) {
    const photos = await getAuditPhotos(auditId);
    let cleaned = 0;

    for (const photo of photos) {
        if (photo.blob && photo.status === 'uploaded') {
            await deletePhotoBlob(photo.id);
            cleaned++;
        }
    }

    console.log(`[DB] Cleaned up ${cleaned} photo blobs for audit ${auditId}`);
    return cleaned;
}
