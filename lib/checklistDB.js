...
/**
 * Helper to deep clone sections with regenerated IDs
 * Generates unique sl_no values to prevent response sharing
 */
function deepCloneSections(sections) {
    const timestamp = Date.now();
    return sections.map((section, sIdx) => ({
        ...section,
        section_id: `section_${timestamp}_${sIdx}`,
        subsections: section.subsections?.map((sub, subIdx) => ({
            ...sub,
            subsection_id: `subsection_${timestamp}_${sIdx}_${subIdx}`,
            items: sub.items?.map((item, iIdx) => ({
                ...item,
                item_id: `item_${timestamp}_${sIdx}_${subIdx}_${iIdx}`,
                // Generate new unique sl_no to prevent response sharing
                sl_no: `${sIdx + 1}.${subIdx + 1}.${iIdx + 1}_${timestamp}`,
            })) || []
        })) || [],
        tables: section.tables?.map((table, tIdx) => ({
            ...table,
            table_id: `table_${timestamp}_${sIdx}_${tIdx}`,
        })) || []
    }));
}

/**
 * Duplicate a single section with new IDs
 * Generates unique sl_no values to prevent response sharing during audit
 */
export function duplicateSection(section, newTitle) {
    const timestamp = Date.now();
    return {
        ...section,
        section_id: `section_${timestamp}`,
        section_title: newTitle || `Copy of ${section.section_title}`,
        subsections: section.subsections?.map((sub, subIdx) => ({
            ...sub,
            subsection_id: `subsection_${timestamp}_${subIdx}`,
            items: sub.items?.map((item, iIdx) => ({
                ...item,
                item_id: `item_${timestamp}_${subIdx}_${iIdx}`,
                // Generate new unique sl_no to prevent response sharing
                sl_no: `${subIdx + 1}.${iIdx + 1}_${timestamp}`,
            })) || []
        })) || [],
        tables: section.tables?.map((table, tIdx) => ({
            ...table,
            table_id: `table_${timestamp}_${tIdx}`,
        })) || []
    };
}

/**
 * Duplicate an entire checklist with a new name
 */
export async function duplicateChecklist(sourceId, newTitle) {
    ...
    const duplicated = {
        ...
        sections: deepCloneSections(source.sections || []),
