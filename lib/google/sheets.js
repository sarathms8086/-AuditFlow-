...
export async function createChecklistSpreadsheet(accessToken, title, auditData, sections, responses) {
    ...
    for (let idx = 0; idx < sections.length; idx++) {
        ...
        const allRows = [...headerRows, ...dataRows, ...tableRows];
        ...
        // Add borders around the entire data area (header row 7 through all data and table rows)
        const totalDataRows = dataRows.length + tableRows.length;
        if (totalDataRows > 0) {
